...
@Injectable()
export class BedsHeroService {
  constructor(private readonly supabaseService: SupabaseService) {}

  async getSettings() {
    const { data, error } = await this.supabaseService
      .getClient()
      .from('beds_hero_settings')
      .select('*')
      .single();

    if (error) throw new NotFoundException('Beds hero settings not found');
    return data;
  }
  ...
  async uploadImage(file: Express.Multer.File) {
    ...
    const filename = `beds-hero/${Date.now()}-${file.originalname}`;
